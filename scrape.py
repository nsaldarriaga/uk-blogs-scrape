...
data_dir = Path('data')
data_dir.mkdir(exist_ok=True) #Indica que estamos ok si la carpeta existe
...
def fetch_entries_for_page(page_number, cutoff_date):
    ...
    no_results_li = entry_list.find('li', {'class': 'no-results'})
...
def parse_article_metadata(article_soup):
    ...
    authors = article_tag.find('a', {'class': 'author'})
